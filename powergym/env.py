...
class Env(gym.Env):
    ...
    def __init__(self, folder_path, info, dss_act=False):
        ...
        self.enable_FDI = info["enable_FDI"]
        self.attack_start_time = info["attack_start_time"]
        self.attack_end_time = info["attack_end_time"]
        self.attack_position1 = info["attack_position1"]
        self.attack_position2 = info["attack_position2"]
        self.attack_position3 = info["attack_position3"]
        self.attack_strength = info["attack_strength"]
        self.lower_limit = info["lower_limit"]
        self.upper_limit = info["upper_limit"]
    ...
    def FDI(self, bus_voltages):
        # if index is outside the range
        #    index is a random choice

        lower_threshold = [self.lower_limit]
        upper_threshold = [self.upper_limit]
        i = 0
        x = 0
        y = 0
        for bus_name in self.attack_position1:
            if bus_voltages[bus_name] < lower_threshold:
                bus_voltages[bus_name] = [v * (1 + self.attack_strength[i]) for v in bus_voltages[bus_name]]
                i = i + 1
            # if bus_voltages[bus_name] > upper_threshold:
            #   bus_voltages[bus_name] = [v * (1 - self.attack_strength[i]) for v in bus_voltages[bus_name]]
            #  i = i + 1
        for bus_name in self.attack_position2:
            if bus_voltages[bus_name] < lower_threshold:
                bus_voltages[bus_name] = [v * (1 + self.attack_strength[x]) for v in bus_voltages[bus_name]]
                x = x + 1
            # if bus_voltages[bus_name] > upper_threshold:
            #   bus_voltages[bus_name] = [v * (1 - self.attack_strength[x]) for v in bus_voltages[bus_name]]
            #  x = x + 1
        for bus_name in self.attack_position3:
            if bus_voltages[bus_name] < lower_threshold:
                bus_voltages[bus_name] = [v * (1 + self.attack_strength[y]) for v in bus_voltages[bus_name]]
                y = y + 1
            # if bus_voltages[bus_name] > upper_threshold:
            #   bus_voltages[bus_name] = [v * (1 - self.attack_strength[y]) for v in bus_voltages[bus_name]]
            #  y = y + 1
    ...
    def step(self, action):
        ...
        ## use the FDI function here to change the original voltage
        ## FDI(bus_voltages, attack_start_time, attack_end_time, attack_position, attack_strength)

        self.obs["true_bus_voltages"] = bus_voltages.copy()
        if (self.enable_FDI):
            bus_voltages = self.FDI(bus_voltages)
    ...
    def reset(self, load_profile_idx=0):
        ...
        for bus_name in self.all_bus_names:
            ...
            bus_voltages[bus_name] = [bus_voltages[bus_name][i] for i in range(len(bus_voltages[bus_name])) if
                                      i % 2 == 0]
        self.obs['bus_voltages'] = bus_voltages
        self.obs["true_bus_voltages"] = bus_voltages
